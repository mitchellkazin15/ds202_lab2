...
Rating <- choco$Rating
?boxplot
boxplot(Rating, horizontal = TRUE)
...
library(tidyverse)
ggplot(data = choco) +
  geom_point(mapping = aes(x = choco$Rating, y = choco$Cocoa.Pct ))
...
?sort
rating_table = table(choco$Company.Location)
rating_counts = subset(rating_table, rating_table > 20)
sorted_rating_counts = sort(rating_counts, decreasing = TRUE)
sorted_rating_counts
top_countries = subset(choco[c(1,3)], choco$Company.Location %in% names(sorted_rating_counts))
avg_rating = aggregate(top_countries$Rating, by = list(unique.values = top_countries$Company.Location, FUN = mean)
barplot(avg_rating$x, names.arg = avg_rating$unique.values, xlab="Country", ylab= "Average Rating", main= "Average Rating by Top 3 Most Reviewed Company Locations", ylim = c(0,5), cex.names = .9)                       
```
